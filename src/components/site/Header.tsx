...
export function Header() {
  ...
  return (
    <>
      ...
      <div className="bg-terracotta-deep py-2 text-center text-[11px] font-medium uppercase tracking-[0.2em] text-primary-foreground sm:text-xs relative overflow-hidden">
        <motion.div 
          animate={{ x: ["-100%", "200%"] }} 
          transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
          className="absolute inset-0 z-0 w-1/3 bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-[-20deg]"
        />
        <span className="relative z-10">✦ Livraison gratuite partout au Maroc · Paiement à la livraison ✦</span>
      </div>
          ...
          <Link to="/" className="flex items-center gap-2 group">
            <motion.div
              animate={{
                ...
                rotate: [0, 15, 0, -15, 0],
                filter: [
                  "drop-shadow(0 0 5px rgba(255, 100, 50, 0.5))",
                  "drop-shadow(0 0 15px rgba(255, 150, 100, 0.8))",
                  "drop-shadow(0 0 5px rgba(255, 100, 50, 0.5))"
                ]
              }}
              transition={{
            ...
            >
              <Sparkles className="h-6 w-6 text-terracotta group-hover:text-gold transition-colors duration-300" />
            </motion.div>
            <div className="flex items-baseline gap-2">
              <span className="font-display text-2xl font-semibold tracking-tight text-terracotta-deep group-hover:glow-text transition-all duration-300">
                Zakia
              </span>
              <span className="text-xs uppercase tracking-[0.3em] text-muted-foreground transition-all duration-300 group-hover:text-terracotta">
                Glow Boutique
              </span>
            </div>
          </Link>
